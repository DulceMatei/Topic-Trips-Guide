...
const routeId = new URLSearchParams(window.location.search).get('traseuId') || 1;
let canEdit = true; // Intotdeauna true pentru trasee personalizate
...
async function fetchLocatiiDisponibile() {
    try {
        const response = await fetch(`/api/traseepersonalizate/locatii-disponibile?traseuId=${routeId}`);
...
function renderLocatiiDisponibile(locatii) {
    ...
    locatii.forEach(loc => {
        ...
        li.innerHTML = `
            <div>
                <b>${loc.name}</b><br>
                <button class="btn btn-sm btn-info mt-2 btn-detalii" data-id="${loc.id}">Detalii & Recenzii</button>
            </div>
            <button class="btn btn-sm btn-success btn-adauga">Adaugă</button>
        `;
        ...
        if (adaugaBtn) {
            adaugaBtn.addEventListener('click', () => {
                ...
                locations.push(loc);
                renderLocationList();
                li.remove(); // elimină vizual din lista de disponibile
    ...
    locations.forEach((location, index) => {
        ...
        contentDiv.innerHTML = `
          ...
          <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            ${canEdit ? `<button class="btn btn-danger btn-sm delete-location" data-location-index="${index}">Șterge</button>` : ''}
          </div>
        `;
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', (e) => {
                ...
                showStatus("Locația a fost ștearsă. Apasă 'Salvează Traseu Personalizat' pentru a actualiza traseul.", "info");
            });
        }
...
async function fetchLocations() {
    try {
        const response = await fetch(`/api/traseepersonalizate/locatii?traseuId=${routeId}`);
...
async function initializeApp() {
    ...
    if (calcRouteBtn && canEdit) {
        calcRouteBtn.addEventListener('click', async () => {
            calcRouteBtn.disabled = true;
            calcRouteBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Se salvează...';
            showStatus('Se recalculează rutele și se salvează traseul personalizat...', 'info');

            await recalculateRoutes();

            const orderedIds = locations.map(loc => loc.id);
            try {
                const response = await fetch(`/api/traseepersonalizate/salveaza-ordine?traseuId=${routeId}`, {
                    method: 'POST',
                    headers: {
                ...
                if (result.success) {
                    showStatus('Traseul personalizat a fost salvat cu succes!', 'success');
                    await fetchLocatiiDisponibile();
                    updateGoogleMapsLink();
                } else {
                    showStatus('A apărut o problemă la salvarea traseului.', 'danger');
                }
            } catch (error) {
                console.error('Eroare la salvare:', error);
                showStatus('Eroare la salvarea traseului personalizat.', 'danger');
            }

            calcRouteBtn.disabled = false;
            calcRouteBtn.innerHTML = '<i class="fas fa-save"></i> Salvează Traseu Personalizat';
        });
    }
}
...
async function captureMapAndDownloadPdf() {
    ...
    try {
        ...
        const dto = {
            titlu: "Traseu Personalizat TopicTripsGuide",
            locatii: locatieDtos,
        ...
        const response = await fetch(`/api/trasee/pdf-complet/${routeId}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        link.download = "Traseu Personalizat.pdf";
        link.click();
